...
2. **Decide whether the cohort should include the domains actually taking citations.** ✅ A cohort is declared (v1, 2026-08-14, five domains from the client). But the most-cited non-Storepro domain across 93 answered runs is `automate-x.nz` (64 source appearances) and it is not in it, nor are `shelvingshopgroup.co.nz` (44) or `stackit.co.nz` (31); meanwhile three cohort members appear 13–20 times each. Who the client considers a competitor and who is actually taking the answer slot are different questions and both are legitimate — but displacement in M6 is measured against the cohort. Adding them writes version 2 with a note, so the history stays readable. **Default if you don't answer:** the client's five stand, and displacement is measured against them only.
...
- **Repo** `CoreshiftHQNZ/engine-optimization`, `dev` and `staging` both at `6823af8`, working tree clean. Working dir `/Users/Ricky/Documents/Claude/Projects/Engine Optimization`. Supabase `xslwvntwrlvqccdupmni`. Railway `engine-optimization`, staging auto-deploys on push to `staging`. `main` does not exist.
...
- **Competitor cohort:** declared v1 on 2026-08-14 by `ricky@coreshifthq.com` — five domains, note *"The client has said that these are their competitors."* `client_context` is versioned and append-only; a change writes v2 rather than editing v1. Of 93 answered probe runs, **43 name a rival, 39 are measured-and-none-cited, 11 remain `null`** (they predate `engine_sources` and must be re-probed, not re-judged).
...
- **Don't** assume a declaration took effect. Storepro's cohort was declared correctly and changed nothing, because `rejudge`'s change-detection did not compare competitor presence and reported "0 of 93 runs would change". Check that the thing you declared actually moved a number.
