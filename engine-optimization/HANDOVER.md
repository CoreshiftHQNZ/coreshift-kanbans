...
2. **Nothing — the cohort is settled.** ✅ Eight domains as of 2026-08-14: the client's five, plus `automate-x.nz`, `shelvingshopgroup.co.nz` and `stackit.co.nz` added on your instruction because they are the most-cited non-Storepro domains in AI answers. v1 preserved, v2 current, both notes on the record.
...
- **Repo** `CoreshiftHQNZ/engine-optimization`, `dev` and `staging` both at `2c2203e`, working tree clean. Working dir `/Users/Ricky/Documents/Claude/Projects/Engine Optimization`. Supabase `xslwvntwrlvqccdupmni`. Railway `engine-optimization`, staging auto-deploys on push to `staging`. `main` does not exist.
...
- **Competitor cohort:** v2, eight domains, declared 2026-08-14 by `ricky@coreshifthq.com`. v1 was the client's own five; v2 added `automate-x.nz`, `shelvingshopgroup.co.nz` and `stackit.co.nz` from measurement. `client_context` is versioned and append-only; a change writes v2 rather than editing v1. Of 93 answered probe runs, **most name a rival under the v2 cohort, and 11 remain `null`** (they predate `engine_sources` and must be re-probed, not re-judged).
