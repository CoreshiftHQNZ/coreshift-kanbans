...
- **Gap found by that tooling, and not yet merged.** The **AI Engineer** channel published a batch of talks squarely inside this window that the first pass missed entirely: *"When Will The Benchmaxxing Plague End?"* (Nick Heiner, Surge AI, 2 Aug) and *"Teaching AI to Find Real Vulnerabilities"* (Prof. David Brumley, Bugcrowd, 1 Aug) are obvious candidates for **Evals** and **AI security** respectively, plus *"MCP Apps: Extending the Frontier"* and *"MCP Tasks (async)"* (both 2 Aug) against the MCP thread, and *"Scaling to Long Horizons"* (31 Jul). Titles and dates are verified; **contents are not**, so nothing was merged — flagged as first business for the next run rather than summarised from a title.
