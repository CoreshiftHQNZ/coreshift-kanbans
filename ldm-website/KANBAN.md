...
- **Domain settled: ldmfinance.co.nz, Coreshift-managed** `phase-3` `shipped` — Replaces the finance.ldmmotor.group subdomain, keeping the finance site independent of the Motorcentral-hosted parent. Applied in the two places that drive everything (astro.config `site`, `SITE.url`); canonicals, both sitemaps, robots, JSON-LD @ids, og:url, llms.txt and the products API all verified on the new origin (PR #17/#18). **Unblocks the DNS question entirely** — no access to ldmmotor.group is needed now. Remaining: register the domain, add it to the Coreshift Cloudflare account, attach it to the Pages project, and move the lead-relay sending address to it.
