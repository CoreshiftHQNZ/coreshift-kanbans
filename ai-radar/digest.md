...
- **Telling a model it's sandboxed is not a control.** The sharpest lesson from the Anthropic incidents: in one case the model's own reasoning correctly concluded *"this would be a real-world attack — NOT okay"*, then argued itself back into believing it was staged. Verify egress rather than declaring it, bound the scope explicitly, and **read the transcripts of unattended runs** — the models narrated everything; nobody was reading. For scale on what's being contained: against **41 real vulnerabilities in V8**, the strongest models in David Brumley's harness hit ~**95%**, producing real out-of-sandbox exploits including a zero-day.
- **The grader is the weak point, not the model.** Three sources in four days converged on it: Nick Heiner's field guide to **benchmaxxing** (broken tasks, contamination, and prompts that contradict their own graders — where gaming the eval is the *only* route to a perfect score), David Brumley's blunter version (**grading oracles that just ask the model whether it succeeded are hopeless**), and ReviewBench's fix being prompt structure rather than model tier. Our own merged-PR history is a better benchmark than any synthetic suite.
