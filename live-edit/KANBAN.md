...
- **Section + theme engine** `shipped` — 22 section types and ~11 theme styles rendered through one shared code path used by both the editor and the edge renderer, so swapping themes never loses content.
